...
module.exports.run = async ({ event, api, Currencies, args }) => {
 const getRandomInt = (min, max) => {
  ...
  if(money < 10000) api.sendMessage("Bạn không đủ tiền !",event.threadID,event.messageID)
    else {
      if(!args[0]) api.sendMessage("Không có số dự đoán.",event.threadID,event.messageID)
         ...
         else api.sendMessage("Con số may mắn là " + number + "\n" + "Chúc bạn may mắn lần sau nhaaa !\n====Lưu ý====\nSau mỗi lần đoán sai, bạn sẽ bị trừ 10000 đô, nếu bạn đúng bạn sẽ nhận lại 50000 đô.",event.threadID, () => Currencies.setData(event.senderID, options = {money: money - 10000}),event.messageID);
      }
    }
  }
};